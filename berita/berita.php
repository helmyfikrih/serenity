...
<td width="9">:
<td colspan="2"><b><?php echo $id_berita;?></b></tr>
<tr>
<td><label for="nama_berita">Nama Berita</label>
<td>:<td width="213"><input name="nama_berita" type="text" id="nama_berita" value="<?php echo $nama_berita;?>" size="20" />
</td>
...
<td colspan="2"><input name="Simpan" type="submit" id="Simpan" class="btn btn-primary" value="<?php echo $label;?>" />
        ...
        <a href="?mnu=berita"><input name="Batal" type="button" id="Batal" class="btn btn-danger" value="Batal" /></a>
</td></tr>
</table>
</form>
...
<?php  
  $sqlq="select distinct(kategori) from `$tbberita` order by `id_berita` desc";
...
?>     
 <h3>Data Berita : <?php echo $kategori;?></h3>
  <div>
<!-- --> 

<br />
Data Berita: 
| <a href="berita/pdf.php"><img src='ypathicon/pdf.png' alt='PDF'></a> 
| <a href="berita/xml.php"><img src='ypathicon/xml.png' alt='XML'></a> 
| <a href="berita/xls.php"><img src='ypathicon/xls.png' alt='XLS'></a> 
...
<?php  
  $sql="select * from `$tbberita` where kategori='$kategori' order by `id_berita` desc";
  $jum=getJum($conn,$sql);
		if($jum > 0){
//--------------------------------------------------------------------------------------------
$batas   = 10;
...
echo"<tr>
				...
				<td>
					...
					<b>Nama berita : $nama_berita</b><br>
...
<img src='$YPATH/$gambar' width='120' height='100' /></a></div>";
				...
				<td><div align='center'>
<a href='?mnu=berita&pro=ubah&kode=$id_berita'><i class='icon-pencil'></i></a>
<a href='?mnu=berita&pro=hapus&kode=$id_berita'><i class='icon-trash' alt='hapus' 
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data $id_berita berhasil disimpan !');document.location.href='?mnu=berita';</script>";}
		else{echo"<script>alert('Data $id_berita gagal disimpan...');document.location.href='?mnu=berita';</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data $id_berita berhasil diubah !');document.location.href='?mnu=berita';</script>";}
		else{echo"<script>alert('Data $id_berita gagal diubah...');document.location.href='?mnu=berita';</script>";}
	}//else simpan
}
?>
...
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data $id_berita berhasil dihapus !');document.location.href='?mnu=berita';</script>";}
	else{echo"<script>alert('Data $id_berita gagal dihapus...');document.location.href='?mnu=berita';</script>";}
}
?>
