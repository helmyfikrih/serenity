...
<script type="text/javascript">
	...
	function PRINTs(v) {
		win = window.open('admin/print.php?kode=' + v, 'win', 'width=1000, height=400, menubar=0, scrollbars=1, resizable=0, location=0, toolbar=0, status=0');
	}
</script>
...
<form action="" method="post" enctype="multipart/form-data">
	<table width="40%">
		<tr>
			...
			<th width="9">:
			<th colspan="2"><b><?php echo $kode_admin; ?></b>
		</tr>
		<tr>
			<td><label for="nama_admin">nama_admin</label>
			<td>:
			<td width="213"><input name="nama_admin" type="text" id="nama_admin" value="<?php echo $nama_admin; ?>" size="20" />
			</td>
...
";
		...
		<tr>
			...
			<td>:
			<td><input name="username" type="username" id="username" value="<?php echo $username; ?>" size="20" /></td>
		</tr>

		<tr>
			<td height="24"><label for="password">password</label>
			<td>:
			<td><input name="password" type="password" id="password" value="<?php echo $password; ?>" size="20" /></td>
		</tr>
		...
		<tr>
			...
			<td>:
			<td><input name="telepon" type="text" id="telepon" value="<?php echo $telepon; ?>" size="15" />
			</td>
		</tr>
...
<img src='$YPATH/$gambar' width='40' height='40' /></a></div>";
			echo "</td>
				<td><div align='center'>
<a href='?mnu=admin&pro=ubah&kode=$kode_admin'><img src='ypathicon/ub.png' alt='ubah'></a>
<a href='?mnu=admin&pro=hapus&kode=$kode_admin'><img src='ypathicon/ha.png' alt='hapus' 
...
'$gambar'
		...
		if ($simpan) {
			...
			echo "<script>alert('Data $kode_admin gagal disimpan...');document.location.href='?mnu=admin';</script>";
		}
	} else {
	...
	`status`='$status',
	`gambar`='$gambar' 
	 where `kode_admin`='$kode_admin0'";
		$ubah = process($conn, $sql);
		if ($ubah) {
			echo "<script>alert('Data $kode_admin berhasil diubah !');document.location.href='?mnu=admin';</script>";
		} else {
			echo "<script>alert('Data $kode_admin gagal diubah...');document.location.href='?mnu=admin';</script>";
		}
	} //else simpan
}
?>
...
if ($_GET["pro"] == "hapus") {
	...
	if ($hapus) {
		echo "<script>alert('Data $kode_admin berhasil dihapus !');document.location.href='?mnu=admin';</script>";
	} else {
		echo "<script>alert('Data $kode_admin gagal dihapus...');document.location.href='?mnu=admin';</script>";
	}
}
?>
