...
";
		...
		<tr>
			<td>
			<td>
			<td colspan="2"><input onclick="return confirm('Apakah data sudah benar?')" name="Simpan" type="submit" id="Simpan" class="btn btn-primary" value="Update Profil" />
...
if (isset($_POST["Simpan"])) {
	...
	if ($ubah) {
		echo "<script>alert('Data $id_tenant Berhasil Diubah !');document.location.href='?mnu=putenant';</script>";
	} else {
		echo "<script>alert('Data $id_tenant Gagal Diubah...');document.location.href='?mnu=putenantupdate&kd=id_tenant';</script>";
	}
} //else simpan
