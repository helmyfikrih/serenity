...
?>     
 <h3>Data Tenant : <?php echo $kategori;?></h3>
  <div>
<!-- --> 

<br />
Data Tenant: 
| <a href="tenant/pdf.php"><img src='ypathicon/pdf.png' alt='PDF'></a> 
| <a href="tenant/xml.php"><img src='ypathicon/xml.png' alt='XML'></a> 
| <a href="tenant/xls.php"><img src='ypathicon/xls.png' alt='XLS'></a> 
...
<img src='$YPATH/$gambar' width='120' height='100' /></a></div>";
				...
				<td>
					...
					<b>Nama Tenant : $nama_tenant (Pj : $id_user)</b><br>
				...
				<td><div align='center'>
<a href='?mnu=tenant&pro=ubah&kode=$id_tenant'><i class='icon-pencil'></i></a>
<a href='?mnu=tenant&pro=hapus&kode=$id_tenant'><i class='icon-trash' alt='hapus' 
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus $nama_tenant pada data tenant ?..\")'></i> </a></div></td>
		...
		else{echo"<tr><td colspan='6'><blink>Maaf, Data tenant belum tersedia...</blink></td></tr>";}
?>
...
if($jmldata>0){
	...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=tenant'>$i</a> ";}
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data $id_tenant berhasil disimpan !');document.location.href='?mnu=tenant';</script>";}
		else{echo"<script>alert('Data $id_tenant gagal disimpan...');document.location.href='?mnu=tenant';</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data $id_tenant berhasil diubah !');document.location.href='?mnu=tenant';</script>";}
		else{echo"<script>alert('Data $id_tenant gagal diubah...');document.location.href='?mnu=tenant';</script>";}
	}//else simpan
}
?>
...
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data $id_tenant berhasil dihapus !');document.location.href='?mnu=tenant';</script>";}
	else{echo"<script>alert('Data $id_tenant gagal dihapus...');document.location.href='?mnu=tenant';</script>";}
}
?>
