	...
	$sama=0;
	  ...
	  $sql="select * from `$tbuser` where password='$password1' and `id_user`='$id_user'";
	   $jums=getJum($conn,$sql);
		...
		echo"<script>alert('Password 1 Salah');document.location.href='?mnu=gantipasswordpu';</script>";		
		...
		echo"<script>alert('Password 3 tidaksesuai');document.location.href='?mnu=gantipasswordpu';</script>";
		}
