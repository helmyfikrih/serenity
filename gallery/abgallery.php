...
<?php  
  $sql="select * from `$tbgallery` where kategori='$ckategori' order by `id_gallery` desc";
  $jum=getJum($conn,$sql);
		if($jum > 0){
//--------------------------------------------------------------------------------------------
$batas   = 10;
...
if($jmldata>0){
	...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=abgallery'>$i</a> ";}
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data $id_gallery berhasil disimpan !');document.location.href='?mnu=abgallery';</script>";}
		else{echo"<script>alert('Data $id_gallery gagal disimpan...');document.location.href='?mnu=abgallery';</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data $id_gallery berhasil diubah !');document.location.href='?mnu=abgallery';</script>";}
		else{echo"<script>alert('Data $id_gallery gagal diubah...');document.location.href='?mnu=abgallery';</script>";}
	}//else simpan
}
?>
...
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data $id_gallery berhasil dihapus !');document.location.href='?mnu=abgallery';</script>";}
	else{echo"<script>alert('Data $id_gallery gagal dihapus...');document.location.href='?mnu=abgallery';</script>";}
}
?>
