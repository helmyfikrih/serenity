...
<td>:<td><input name="keterangan" type="text" id="keterangan" value="<?php echo $keterangan;?>" size="20" />
</td>
</tr>
...
<td colspan="2"><input name="Simpan" type="submit" id="Simpan" class="btn btn-primary" value="<?php echo $label;?>" />
        ...
        <a href="?mnu=gallery"><input name="Batal" type="button" id="Batal" class="btn btn-danger" value="Batal" /></a>
</td></tr>
</table>
</form>
-->
...
<div id="accordion">
<!-- </div> -->
 <h3>Data Gallery : <?php echo $kategori;?></h3>
  <div>
<!-- --> 
...
<img src='$YPATH/$gambar' width='100' height='100' /></a></div>";
				...
				<td><div align='center'>
<a href='?mnu=gallery&pro=ubah&kode=$id_gallery'><i class='icon-pencil'></i></a>
<a href='?mnu=gallery&pro=hapus&kode=$id_gallery'><i class='icon-trash' alt='hapus' 
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus $nama_gallery pada data gallery ?..\")'></i> </a></div></td>
		...
		else{echo"<tr><td colspan='6'><blink>Maaf, Data gallery belum tersedia...</blink></td></tr>";}
?>
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=gallery'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=gallery'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=gallery'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data $id_gallery berhasil disimpan !');document.location.href='?mnu=gallery';</script>";}
		else{echo"<script>alert('Data $id_gallery gagal disimpan...');document.location.href='?mnu=gallery';</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data $id_gallery berhasil diubah !');document.location.href='?mnu=gallery';</script>";}
		else{echo"<script>alert('Data $id_gallery gagal diubah...');document.location.href='?mnu=gallery';</script>";}
	}//else simpan
}
?>
...
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data $id_gallery berhasil dihapus !');document.location.href='?mnu=gallery';</script>";}
	else{echo"<script>alert('Data $id_gallery gagal dihapus...');document.location.href='?mnu=gallery';</script>";}
}
?>
