...
<script type="text/javascript">
	function PRINT() {
		win = window.open('peserta/abprint<?php echo $ckategori ?>.php', 'win', 'width=1000, height=400, menubar=0, scrollbars=1, resizable=0, location=0, toolbar=0, status=0');
	}
</script>
		...
		<form action="" method="post" enctype="multipart/form-data">
			<table width="40%">
				...
				<tr>
					...
					<td>:
					<td>
						<select name="id_seminar" required>
							...
							foreach ($q as $d) {
								...
								$jam2 = $d["jam"];
								echo "<option value='$id_seminar0' ";
								if ($id_seminar0 == $id_seminar) {
									echo "selected";
								}
								echo ">$id_seminar0 - $nama_seminar0  </option>";
							}
							?>
						</select>
					</td>
					...
					<td>
						<select name="id_tenant" required>
							...
							foreach ($q as $d) {
								...
								echo "<option value='$id_tenant0' ";
								if ($id_tenant0 == $id_tenant) {
									echo "selected";
								}
								echo ">$id_tenant0 - $nama_tenant  </option>";
							}
							?>
						</select>
					</td>
				...
				<tr>
					...
					<td>:
					<td><input name="tanggal" type="text" id="tanggal" value="<?php echo $tanggal; ?>" size="25" />
					</td>
				</tr>
				...
				<tr>
					...
					<td>:
					<td><input name="jam" type="text" id="jam" value="<?php echo $jam; ?>" size="25" />
					</td>
				</tr>
				...
				<tr>
					...
					<td>:
					<td colspan="2">
						<input type="radio" name="status" id="status" checked="checked" value="Terdaftar" <?php if ($status == "Terdaftar") {
																												echo "checked";
																											} ?> />Terdaftar
						<input type="radio" name="status" id="status" value="Hadir" <?php if ($status == "Hadir") {
																						echo "checked";
																					} ?> />Hadir
						<input type="radio" name="status" id="status" value="Batal" <?php if ($status == "Batal") {
																						echo "checked";
																					} ?> />Batal
...
onClick='return confirm(\"Anda Yakin Akan Menghapus Data Peserta $id_peserta?\")'></i></a></div></td>
			...
			if ($jmldata > 0) {
				...
				for ($i = 1; $i <= $jmlhal; $i++)
					if ($i != $page) {
						echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=abpeserta'>$i</a> ";
					} else {
